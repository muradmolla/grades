...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create grades-16e1
  fly apps create grades-16e1-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  flyctl secrets set SESSION_SECRET=$(openssl rand -hex 32) --app grades-16e1
  flyctl secrets set SESSION_SECRET=$(openssl rand -hex 32) --app grades-16e1-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'grades-16e1-staging' does not match app name in config file 'grades-16e1'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  flyctl postgres create --name grades-16e1-db
  flyctl postgres attach --postgres-app grades-16e1-db --app grades-16e1

  flyctl postgres create --name grades-16e1-staging-db
  flyctl postgres attach --postgres-app grades-16e1-staging-db --app grades-16e1-staging
  ```
